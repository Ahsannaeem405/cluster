...
class ServiceController extends Controller
{
    ...
    public function invite_user(Request $request)
    {
        ...
        for ($i = 0; $i < count($request->userid); $i++) {
            ...
            $geteve = EventJoin::where('user_id', $request->userid[$i])->where('event_id', $request->eventID)->where('cluster_id', $request->clusterID)->count();

            if ($geteve == 0) {

                $eventt = new EventJoin();

                $eventt->user_id = $request->userid[$i];
                $eventt->event_id = $request->eventID;
                $eventt->cluster_id = $request->clusterID;
                $eventt->status = 'joined';
                $eventt->save();
            }
